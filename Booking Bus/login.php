...
<?php
	if(isset($_POST['user'])){
		$name = $_POST['user'];
		$query = "SELECT login., Customer. FROM login INNER JOIN Customer WHERE login.UserName='$name' ";
		$a = $konek->query($query);
		$fetch = $a->fetch_assoc();
		$count = $a->num_rows;
		if($count > 0 & $count = 1){
?>
			<img src="images/user/<?php echo $fetch['UserImage']; ?>" id="usr-img">
...
<?php
		...
		if($hitung > 0 and $hitung = 1){
			...
			$name = $_POST['username'];
			$query = "SELECT login., Customer. FROM login LEFT JOIN Customer WHERE login.UserName='$name' ";
			$a = $konek->query($query);
			$fetch = $a->fetch_assoc();
?>
			<img src="images/user/<?php echo $fetch['UserImage']; ?>" id="usr-img">
			<p id="name"><?php echo $fetch['name']; ?></p>
